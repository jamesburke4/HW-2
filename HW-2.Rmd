...
#                    "STEM" = "computer / hardware / software",
                    "travel / transportation" = "transportation",
                    "travel / transportation" = "hospitality / travel"
                ),    
...
ggplot(data = profiles_jobs, aes(x = fct_reorder(f=job, x=n, fun=max), y = n, fill = sex)) + 
  ...
  labs(x = "Job", y = "Number of Users", title = "Jobs by Gender") +
  theme(axis.text.x = element_text(angle = -60, hjust = 0))
```

Observing the distribution of jobs from the sample, as well as their breakdown by gender, a couple points stand out. First of all, The fact that "computer / hardware / software" and "science / tech / engineering" are two of the most common occupations should not be surprising, but the number of men in the sample compared to the number of women is drastic. Besides "construction / craftsmanship" and "military," each representing a small group of users, no other occupation comes close to the proportion of men that these two show. Additionally, only three categories include more men than women, "clerical / administrative," "education / academia," and "medicine / health." We know our sample is made up of more men than women, but with the significant difference of gender in the aforementioned occupations, I thought more than three categories would have a women majority. Lastly, the number of people who described their jobs as “other” surprised me. The number of categories provided seems to be sufficient in covering the wide array of occupations, so I would think that the "other" category should be much smaller.
...
The boxplots show a slight edge in median income for men, as well as a higher 1st and 3rd quartile. Interestingly, the minimum and 1st quartile for women are the same (20,000). These plots do not show outliers that represent incomes greater than 150,000, but a deeper look at these 718 people reveals an interesting trend.
...
employed <- profiles_inc %>% 
  filter(income > 150000) %>% 
  mutate(Employment = ifelse(job == "not working", "Not Employed", 
                             ifelse(job == "undisclosed" | job == "other", 
                                    "Undisclosed / Other", "Specified Occupation")
                             )
         ) %>%
  count(Employment)
employed %>% kable()
...
As the above table shows, only 490 of 718 people who reported more income of more than 150,000 specified their occupation in their profile. That leaves 228 people who did not specify their job but reported high levels of income. This includes 85 people who reported not having a job reporting income of more than 150,000. This questions the accuracy of income reporting from the sample, especially those who didn't report employment but reported income.
...
ggplot(data = orient_chart_1, aes(x = sex, y = n, fill = orientation)) + 
...
ggplot(data = profiles_drinks, aes(x = fct_reorder(f=orientation, x=n, fun=max), y = n, fill = drinks)) + 
  geom_bar(stat = "identity", position = "fill") +
